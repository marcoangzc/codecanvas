...
| 2️⃣ JavaScript 编程思维 | 变量、函数、DOM、事件、待办清单项目 | ✅ 已上线 |
| 3️⃣ React 框架 | 组件、状态、副作用、天气看板项目 | ✅ 已上线 |
| 4️⃣ Node.js 与 API | HTTP 深入、接口设计、鉴权、留言板项目 | ✅ 已上线 |
| 5️⃣ 数据库基础 | SQL 建模、查询、ORM、图书查询项目 | ✅ 已上线 |
| 6️⃣ 全栈实战 | 需求到上线的完整流程、毕业挑战 | ✅ 已上线 |

## ✨ 四大互动形式

1. **实时代码演练场** — 左边写代码右边 iframe 实时预览，带控制台输出；`mode="react"` 还能在浏览器里直接跑 JSX（无需构建工具）
2. **动画概念图** — 网页诞生之旅（HTTP 全流程）、HTML 文档解剖图、DOM 树生长动画、变量赋值可视化
3. **分步交互讲解** — 盒模型实验室、Flexbox 实验室、响应式断点实验室、选择器小游戏
4. **进度与成就** — XP 经验等级、10 枚徽章（六大模块各有通关徽章）、课程完成度（保存在浏览器 localStorage）
...
- 真实留言板 API：`GET/POST /api/guestbook`（模块 4 起的前后端联调练习场，数据落盘 `.data/`）
